...
utilities.deleteEmptyProperties = function (obj) {
  for (const key of Object.keys(obj)) {
    ...
    if (!['default', 'example', 'security'].includes(key)) {
      ...
      if (utilities.isObject(obj[key]) && !utilities.hasProperties(obj[key])) {
        delete obj[key];
      }
    }
  }
...
utilities.hasJoiChildren = function (joiObj) {
  ...
  const byId = joiObj?._ids?._byId;
  const byKey = joiObj?._ids?._byKey;
...
utilities.hasJoiDescription = function (joiObj) {
  ...
  return !!joiObj?._flags?.description;
};
...
utilities.getJoiMetaProperty = function (joiObj, propertyName) {
  ...
  if (utilities.isJoi(joiObj) && utilities.hasJoiMeta(joiObj)) {
    return joiObj.$_terms.metas.findLast((item) => item[propertyName])?.[propertyName];
  }
...
utilities.getJoiLabel = function (joiObj) {
  // old version
  /* $lab:coverage:off$ */
  const label = joiObj?._settings?.language?.label;
  ...
  /* $lab:coverage:on$ */
  // Joi > 10.9
  return joiObj?._flags?.label || null;
};
...
utilities.assignVendorExtensions = function (target, source) {
  ...
  for (const sourceProperty of Object.keys(source)) {
    if (sourceProperty.startsWith('x-') && sourceProperty.length > 2) {
      ...
      target[sourceProperty] = source[sourceProperty] || null;
    }
  }
