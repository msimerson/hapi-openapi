...
builder.getSwaggerJSON = async (settings, request) => {
  ...
  settings = { ...builder.default, ...settings };
  ...
  if (settings.customSwaggerFile) {
    return { ...settings.customSwaggerFile, ...out };
  }
  ...
  if (settings.debug) {
    Validate = Validate || require('../lib/validate');
...
builder.dereference = async (schema) => {
  let JSONDeRef;
  try {
    JSONDeRef = require('@apidevtools/json-schema-ref-parser');
  } catch {
    throw new Error(
      `The "deReference" option requires the "@apidevtools/json-schema-ref-parser" package. Install it with: npm install @apidevtools/json-schema-ref-parser`
    );
  }
 ...
 */
internals.getProxyHeader = (request, name) => {
...
internals.getServerUrl = (request, settings) => {
 ...
 * @return {Object}
 */
internals.removeNoneSchemaOptions = (options, settings) => {
  const out = { ...options };
  [
    'debug',
    'documentationPath',
