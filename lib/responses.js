...
internals.responses.prototype.build = function (
  ...
  if (Utilities.hasProperties(statusSchemas)) {
    for (const [key, schema] of Object.entries(statusSchemas)) {
      // name, joiObj, parameterType, useDefinitions, isAlt
      out[key] = this.getResponse(key, schema, null, useDefinitions, isAlt);
    }
  }
...
internals.responses.prototype.optionOverride = function (
  ...
  for (const [key, userSchema] of Object.entries(userDefindedSchemas)) {
    // create a new object by cloning - dont modify user definded objects
    let out = { ...userSchema };

    // test for any JOI objects
    if (userSchema?.schema && Utilities.isJoi(userSchema.schema)) {
      out = this.getResponse(key, userSchema.schema, useDefinitions, isAlt);
      out.description = userSchema.description;

      if (userSchema.headers) {
        out.headers = userSchema.headers;
      }
    ...
    if (!discoveredSchemas[key] && out) {
      ...
      for (const [item, value] of Object.entries(out)) {
        if (value !== undefined) {
...
internals.responses.prototype.getResponse = function (statusCode, joiObj, useDefinitions) {
  let out = {
    description: joiObj?._flags?.description,
