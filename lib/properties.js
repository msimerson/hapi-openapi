...
exports =
  ...
  internals.properties =
    function (settings, definitionCollection, altDefinitionCollection, definitionCache) {
      ...
      // merge
      this.propertyMap = { ...this.simpleTypePropertyMap, ...this.complexTypePropertyMap };
...
internals.properties.prototype.parseProperty = function (name, joiObj, parent, parameterType, useDefinitions, isAlt) {
  ...
  if (joiObj?._flags?.presence === 'forbidden') {
    return undefined;
  }
...
internals.properties.prototype.setRequiredAndOptionalOnParent = function (name, parent, joiObj) {
  ...
  if (joiObj?._flags?.presence) {
    ...
    if (joiObj._flags.presence === 'required') {
      if (!parent.required.includes(name)) {
        parent.required.push(name);
      }
    }

    if (this.settings.OAS === 'v2' && joiObj._flags.presence === 'optional') {
  ...
  if (describe.whens && describe.whens.length > 0) {
    describe.whens.forEach((test) => {
      if (test?.then?.flags?.presence === 'required') {
        if (parent.required === undefined) {
          parent.required = [];
        }
...
internals.properties.prototype.parsePropertyMetadata = function (property, name, parent, joiObj) {
  ...
  property.description = joiObj?._flags?.description;
  property.notes = joiObj?.$_terms?.notes;
  property.tags = joiObj?.$_terms?.tags;
  ...
  if (this.settings.xProperties === true) {
    ...
    const exampleObj = joiObj?.$_terms?.examples?.[0];
    /* c8 ignore start */
    ...
    const xMeta = joiObj?.$_terms?.metas?.[0];
    if (xMeta) {
      const meta = { ...xMeta };
  ...
  let propertyDefault = joiObj?._flags?.default;
...
internals.properties.prototype.parseNumber = function (property, joiObj) {
  ...
  if (Array.isArray(describe.metas)) {
    const meta = describe.metas.find((meta) => typeof meta.format === 'string');
    if (meta) {
...
internals.properties.prototype.parseDate = function (input, joiObj) {
  const dateFormat = joiObj?._flags?.format;
...
internals.properties.prototype.parseArray = function (property, joiObj, name, parameterType, useDefinitions, isAlt) {
  ...
  // if there are multiple types, use anyOf, otherwise grab the first one
  const arrayItemTypes = joiObj.$_terms.items;
  const firstArrayItem = Utilities.first(arrayItemTypes);

  if (arrayItemTypes.length > 1 && this.settings.OAS === 'v3.0') {
    delete property.items.type;
    property.items.anyOf = arrayItemTypes.map((item) => {
      return this.parseProperty(Utilities.getJoiLabel(item), item, property, parameterType, useDefinitions, isAlt);
    });
  } else if (firstArrayItem) {
    // get name of item if it has one
    let itemName;
    if (Utilities.getJoiLabel(firstArrayItem)) {
      itemName = Utilities.getJoiLabel(firstArrayItem);
    }
    ...
    if (this.simpleTypePropertyMap[firstArrayItem.type.toLowerCase()]) {
...
internals.properties.prototype.parseAlternatives = function (property, joiObj, name, parameterType, useDefinitions) {
  ...
  if (joiObj?.$_terms?.matches?.[0]?.schema) {
    if (this.settings.OAS === 'v2') {
      ...
      if (this.settings.xProperties === true) {
        ...
        property['x-alternatives'] = [...altArray];
      }
    } else {
