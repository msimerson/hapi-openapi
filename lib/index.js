              ...
              options: Hoek.applyToDefaults(
                {
                  ...
                  handler: (request, h) => {
                    ...
                    return h.response(buildDocsPage(pageSettings)).type('text/html');
                  },
              ...
              options: Hoek.applyToDefaults(
                {
                  ...
                  handler: (request, h) => {
                    const pageSettings = buildPageSettings(settings, request, server);
                    return h.response(pageSettings).type('application/json');
                  },
                  plugins: settings.documentationRoutePlugins
                },
                validateOption
 ...
 * @return {Object}
 */
const buildPageSettings = function (settings, request, server) {
  const routePrefix = server.realm.modifiers.route.prefix;
  const pageSettings = Hoek.clone(settings);
 ...
 * @return {string}
 */
const buildDocsPage = function (settings) {
  const title = (settings.info && settings.info.title) || '';
  const uiPath = settings.swaggerUIPath;
...
<html lang="en">
  ...
  <body>
    ...
    <script id="hapi-swagger-config" type="application/json">${config}</script>
    <script>
      function getUrlVars() {
        const vars = [];
        let hash;
      ...
      window.onload = function () {
        const cfg = JSON.parse(document.getElementById('hapi-swagger-config').textContent);
        ...
        const swaggerOptions = {
          ...
          onComplete: function () {
            if (cfg.uiCompleteScript) {
              ...
              (new Function(cfg.uiCompleteScript))();
            }
          },
          tryItOutEnabled: cfg.tryItOutEnabled
        };

        Object.assign(swaggerOptions, cfg.uiOptions);
