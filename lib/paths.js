...
internals.paths.prototype.build = function (routes) {
  ...
  routes.forEach((route) => {
    const routeOptions =
      route?.settings?.plugins?.['@msimerson/hapi-openapi'] ?? route?.settings?.plugins?.['hapi-swagger'] ?? {};
    ...
    if (routeOptions?.validate?.query) {
      routeData.queryParams = Utilities.toJoiObject(routeOptions.validate.query);
    }

    if (routeOptions?.validate?.params) {
      routeData.pathParams = Utilities.toJoiObject(routeOptions.validate.params);
    }

    if (routeOptions?.validate?.headers) {
      routeData.headerParams = Utilities.toJoiObject(routeOptions.validate.headers);
    }

    if (routeOptions?.validate?.payload) {
      // has different structure, just pass straight through
      routeData.payloadParams = routeOptions.validate.payload;
    ...
    routesData.push(...internals.getMultiMethodRoutes(routeData, this.settings.wildcardMethods));
  });
...
internals.createRouteMetaData = function (route, routeOptions) {
  return {
    ...
    authAccess: route?.settings?.auth?.access || null,
    queryParams: route?.settings?.validate?.query,
    pathParams: route?.settings?.validate?.params,
    payloadParams: route?.settings?.validate?.payload,
    responseSchema: route?.settings?.response?.schema,
    responseStatus: route?.settings?.response?.status,
    headerParams: route?.settings?.validate?.headers,
    consumes: routeOptions?.consumes || null,
    produces: routeOptions?.produces || null,
    responses: routeOptions?.responses || null,
    payloadType: routeOptions?.payloadType || null,
    security: routeOptions?.security || null,
    order: routeOptions?.order || null,
    deprecated: routeOptions?.deprecated || null,
    id: routeOptions?.id || null,
    groups: route.group
  };
};
    ...
    if (Utilities.hasJoiChildren(pathJoi)) {
      ...
      pathStructures.parameters.forEach((item) => {
        ...
        if (!item.required) {
          this.settings.log(
            ['validation', 'warning'],
            `The ${path} params parameter {${item.name}} is set as optional. This will work in the UI, but is invalid in the swagger spec`
          );
        }
      });
    } else {
...
internals.paths.prototype.buildOpenApiRoutes = function (routes) {
  ...
  routes.forEach((route) => {
    ...
    // OAS v3.0: wrap object-typed query params with content-based JSON encoding
    out.parameters = this.contentEncodeObjectQueryParams(out.parameters, queryJoi);
...
/**
 * OAS v3.0: converts object-typed query parameters to use content-based JSON encoding.
 * Creates named definitions for labeled Joi schemas and wraps with
 * content: { 'application/json': { schema: { $ref } } }
 *
 * @param  {Array} parameters - assembled route parameters
 * @param  {Object} queryJoi - the original Joi schema for query validation
 * @return {Array} parameters with object query params content-encoded
 */
internals.paths.prototype.contentEncodeObjectQueryParams = function (parameters, queryJoi) {
  if (this.settings.OAS === 'v2' || !parameters || !parameters.length) {
    return parameters;
  }

  return parameters.map((param) => {
    if (param.in !== 'query' || !param.schema || param.schema.type !== 'object') {
      return param;
    }

    // Find the Joi child schema to get its label for the definition name
    let label = null;
    if (queryJoi && queryJoi.$_terms && queryJoi.$_terms.keys) {
      const joiChild = queryJoi.$_terms.keys.find((k) => k.key === param.name);
      if (joiChild && joiChild.schema) {
        label = Utilities.getJoiLabel(joiChild.schema);
      }
    }

    let schema;
    if (label) {
      // Create a named definition in components/schemas and reference via $ref
      const definitionCollection = this.properties.getDefinitionCollection(false);
      const refName = this.properties.definitions.append(label, param.schema, definitionCollection, this.settings);
      schema = { $ref: `#/components/schemas/${refName}` };
    } else {
      // Use inline schema when no label is available
      schema = param.schema;
    }
